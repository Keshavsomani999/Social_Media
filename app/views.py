...
def index(request):
    ...
    return render(request,'index.html',{'user_profile':user_profile,'user_object':user_object,'posts':feed_list,'suggestions_username_profile_list':suggestions_username_profile_list[:4]})
...
def upload(request):
    ...
    if request.method == 'POST':
        ...
        return render(request,'upload.html',{'user_profile':user_profile,'user_object':user_object})
    ...
    if request.method == "POST":
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                #log user in and redirect to setting
...
def settings(request):
    ...
    if request.method == 'POST':
        if request.FILES.get('image')==None:
            ...
            user_profile.background = background
            user_profile.save()
        
        if request.FILES.get('bg')==None:
